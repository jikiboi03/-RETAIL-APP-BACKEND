	...
	if (tableID == "items-table") {
		...
		table = $("#packages-table").DataTable({
			...
			rowCallback: function(row, data, index) {
				...
				if (row_count == 0) {
					$node.css("background-color", "#ccccff");
				} else if (best_selling.length > 20) {
					// if there are words such as rank, star symbol, etc. (more than 14 chars automatically)
					function isOdd(num) {
						return num % 2;
					}
		...
		if (trans_status == "CLEARED") {
			// - 1
			url = "showlist-transactions/1";
		} else if (trans_status == "CANCELLED") {
			// - 2
...
function complete_po() {
	if (confirm("Are you sure to set this PO as completed?")) {
		// resetting errors in form validations
		$(".form-group").removeClass("has-error"); // clear error class
		$(".help-block").empty(); // clear error string
...
function cancel_po() {
	if (confirm("Are you sure to set this PO as cancelled?")) {
		// resetting errors in form validations
		$(".form-group").removeClass("has-error"); // clear error class
		$(".help-block").empty(); // clear error string
...
$("#form_set_payment").submit(function(event) {
	// -------------------------------- EXPIREMENTAL FUNCTION (Fixes dismissed modal when add_cash_input buttons are clicked)
...
function print_bill_out(id) {
	// ---> calling for print billout
	//Ajax Load data from ajax
	$.ajax({
		url: "../edit-transaction/" + id,
		type: "GET",
...
function reprint_last_trans_receipt() {
	// ---> calling for the Add Modal form
	//Ajax Load data from ajax
	$.ajax({
...
$("#discount_prod_id").change(
	function() // display product's original price when selected on the product discount form
	{
		var prod_id = $('[name="prod_id"]').val();
...
$("#discount_pack_id").change(
	function() // display package's original price when selected on the package discount form
	{
		var pack_id = $('[name="pack_id"]').val();
...
function toggle_pw_mask() {
	// password mask/unmask toggle button
...
function create_po() {
	// calling for create po function
	url = "create-po";
	$form = "#form";
		...
		success: function(data) {
			if (data.status) {
				...
				if (save_method == "add-item") {
					...
					details =
						"New product discount added: P" + $('[name="prod_id"]').val();
					...
					details =
						"New package discount added: G" + $('[name="prod_id"]').val();
					...
					details =
						"New payment added to Loan ID: L" +
						$('[name="loan_id"]').val() +
						" of Client: " +
					...
					details =
						"New interest added to Loan ID: L" +
						$('[name="loan_id"]').val() +
						" of Client: " +
						$('[name="client_name"]').val();
					...
					details =
						"New loan adjustment to Loan ID: L" +
						$('[name="loan_id"]').val() +
						" of Client: " +
						$('[name="client_name"]').val();
					...
					details =
						"Capital adjustment updated to: P" + $('[name="capital_id"]').val();
...
function set_system_log_three(log_type, details) {
	...
	$.ajax({
		url:
			"../../../Logs/Logs_controller/ajax_add/" + log_type + "/" + cleanString,
		type: "POST",
		dataType: "JSON",
		success: function(data) {},
		error: function(jqXHR, textStatus, errorThrown) {
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Product deleted P" + id + ": " + name;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Package deleted G" + id + ": " + name;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Category deleted C" + id + ": " + name;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Table deleted T" + id + ": " + name;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Discount deleted D" + id + ": " + name;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Product item deleted P" + idone + ": I" + idtwo;
	...
	if (confirm("Are you sure to delete this data?")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var details = "Package product deleted G" + idone + ": P" + idtwo;
...
function set_report_transactions() {
	...
	if (report_type == "alltime") {
		...
		if (trans_report_type == "trans-all") {
			...
			window.open(
				"transactions-report-monthly/CANCELLED/" + year + "/" + month
			);
		} else if (trans_report_type == "trans-refunded") {
			window.open("transactions-report-monthly/REFUNDED/" + year + "/" + month);
		}
	} // for custom range dates
	else {
		...
		if (date_from == "" || date_to == "") {
			...
			if (trans_report_type == "trans-all") {
				window.open(
					"transactions-report-custom/ALL/" + date_from + "/" + date_to
				); //  if transaction status is set to 'ALL'
			} else if (trans_report_type == "trans-cleared") {
				window.open(
					"transactions-report-custom/CLEARED/" + date_from + "/" + date_to
				);
			} else if (trans_report_type == "trans-cancelled") {
				window.open(
					"transactions-report-custom/CANCELLED/" + date_from + "/" + date_to
				);
			} else if (trans_report_type == "trans-refunded") {
				window.open(
					"transactions-report-custom/REFUNDED/" + date_from + "/" + date_to
				);
			}
		}
	}
	...
	if (report_type == "alltime") {
		...
		if (date_from == "" || date_to == "") {
			...
			if (menu_items_report_type == "menu-products") {
				...
				window.open(
					"menu-top-selling-report-custom/" + date_from + "/" + date_to
				);
			}
		}
	}
...
function set_report_users() {
	...
	if (report_type == "alltime") {
		...
		if (users_report_type == "users-all") {
			...
			window.open(
				"users-report-type-monthly/administrator/" + year + "/" + month
			);
		} else if (users_report_type == "users-cashier") {
		...
		if (date_from == "" || date_to == "") {
			...
			if (users_report_type == "users-all") {
				window.open(
					"users-report-type-custom/All/" + date_from + "/" + date_to
				); //  if transaction status is set to 'ALL'
			} else if (users_report_type == "users-admin") {
				window.open(
					"users-report-type-custom/administrator/" + date_from + "/" + date_to
				);
			} else if (users_report_type == "users-cashier") {
				window.open(
					"users-report-type-custom/cashier/" + date_from + "/" + date_to
				);
			} else if (users_report_type == "users-staff") {
				window.open(
					"users-report-type-custom/staff/" + date_from + "/" + date_to
				);
			}
		}
	}
...
if (document.getElementById("container-products-category")) {
	...
	Highcharts.chart("container-products-category", {
		...
		title: {
			text:
				"Product categories sales data ( Total: ₱ " +
				cat_total_sales
					.toFixed(2)
					.toString()
					.replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
		...
		tooltip: {
			pointFormat:
				"{series.name} Percentage: <b>{point.percentage:.1f}%</b><br>{point.details}"
		},
		plotOptions: {
			pie: {
...
if (document.getElementById("container-top-selling-menu-items")) {
	...
	Highcharts.chart("container-top-selling-menu-items", {
		...
		title: {
			text:
				"Total menu items sold of top selling products/packages ( Total: " +
				menu_total_sold +
				" )"
		},
		subtitle: {
...
if (document.getElementById("container-users-cashier")) {
	...
	for (i = 0; i <= cashier_index; i++) {
		...
		cashier_trans_count[i] = parseInt(
			$('[name="cashier_trans_count' + i + '"]').val()
		);
		cashier_net_sales[i] = parseFloat(
			$('[name="cashier_net_sales' + i + '"]').val()
		);
	...
	Highcharts.chart("container-users-cashier", {
		...
		title: {
			text:
				"Cashier<br>transactions<br> ( Total: " + cashier_total_trans + " )",
			align: "center",
		...
		tooltip: {
			pointFormat:
				"{series.name}: <b>{point.percentage:.1f}%</b><br>{point.details}"
		},
		plotOptions: {
			pie: {
				dataLabels: {
...
if (document.getElementById("container-users-staff")) {
	...
	for (i = 0; i <= staff_index; i++) {
		...
		staff_trans_count[i] = parseInt(
			$('[name="staff_trans_count' + i + '"]').val()
		);
		staff_net_sales[i] = parseFloat(
			$('[name="staff_net_sales' + i + '"]').val()
		);
